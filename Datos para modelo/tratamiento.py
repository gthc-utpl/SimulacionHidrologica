    ...
    try:
        ...
        print('Obteniendo Datos de los Archivos Raster')
        nlat = np.shape(a)
        nlon = np.shape(a)
        print('Obteniendo Datos de los Archivos Raster')
        ...
        #Obtener las coordenadas de las esquinas y resolucion del pixel
        x_origin = b[0]
        y_origin = b[3]
        x_res = b[1]
        y_res = b[5]
        print('hasta aqui todo bien')
        #Obtener el acnho y altura del pixel
        width = ds.RasterXSize
        height = ds.RasterYSize
        #Calcular las coordendas de la esquina derecha
        x_end = x_origin + (width * x_res)
        y_end = y_origin + (height * abs(y_res))
        basedate = dt.datetime(1980,1,1,0,0,0)
        # create NetCDF file
        nco = netCDF4.Dataset('C:/Qgis/netCDFiles/time_series.nc','w',clobber=True)
        print('hasta aqui todo bien')
        ...
        nco.title = "Caudales"
        nco.history = "created by Andrés Vallejos"
        ...
        pat = re.compile('Datos-[0-9]{4}-[0-9]{2}-[0-9]{2}.tif')
        itime=0
